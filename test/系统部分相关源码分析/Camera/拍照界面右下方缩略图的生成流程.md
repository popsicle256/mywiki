...
# 1 点击拍照完成后更新缩略图
点击拍照，拍照完成后，会直接实时生成图片和对应的缩略图并将缩略图更新到拍照界面的右下角，该缩略图是通过**图片byte数据**转换成bitmap形式的。
...
大致过程描述：
	...
	当相机捕获完成后，会回调ImageReader.postEventFromNative()，该方法当图像可用时会触发之前在takePicture中注册的**图像可用监听器**
	...
	CameraAppUI中调用ThumbnailViewManager，将传入的bitmap缩略图更新到相机界面视图中（右下角）  
...
`mCaptureDataCallback`的生成是在`PhotoDevice2Controller.takePicture()`中（**1.7**），
	而此处的`takePicture()`又是由`PhotoMode`所调用的（**1.8**），所以这里的`mCaptureDataCallback.onPostViewCallback()`即为`PhotoMode.onPostViewCallback()`.
